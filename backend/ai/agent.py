...
try:
    ...
    _env_path = Path(__file__).resolve().parents[2] / ".env"  # Echo/.env
...
_SYSTEM = """\
You are the AI Director inside Echo, a professional video editor.
...
NOTE ON AUDIO TRACKS: Echo uses a mono-track architecture — audio clips can live on
...
INDEXING CONTROL:
Echo auto-starts Vision+Whisper indexing whenever a video/image is downloaded.

import React from 'react'
import ReactDOM from 'react-dom/client'
import App from './App'
import './index.css'

// ── Port discovery ─────────────────────────────────────────────────────────────
// Three-layer strategy so the port is NEVER missed:
//  1. onBackendPort (push)  — main process sends it when Python prints the port
//  2. getPort() (pull)      — renderer polls main process after load (catches race)
//  3. HTTP probe fallback   — plain browser dev without Electron

;(window as any).__ECHO_PORT__ = null

function broadcastPort(port: number) {
  if ((window as any).__ECHO_PORT__ === port) return   // already known
  ;(window as any).__ECHO_PORT__ = port
  window.dispatchEvent(new CustomEvent('echo:port', { detail: port }))
  console.log('[Echo] Backend port:', port)
}

const eAPI = (window as any).electronAPI

if (eAPI?.onBackendPort) {
  // Layer 1 — push: backend:port IPC from main process
  eAPI.onBackendPort((port: number) => broadcastPort(port))

  // Layer 2 — pull: ask main process for the stored port (handles timing race)
  // Polls every 500 ms for up to 30 seconds (60 attempts)
  let pollAttempts = 0
  const poll = async () => {
    if ((window as any).__ECHO_PORT__) return   // already known via push
    if (pollAttempts++ > 60) return             // give up after 30 s — push path still active
    try {
      const port: number | null = await eAPI.getPort()
      if (port) { broadcastPort(port); return }
    } catch { return }   // handler not registered in old build — stop polling
    // Slow down after first 10 attempts
    const delay = pollAttempts < 10 ? 500 : 1000
    setTimeout(poll, delay)
  }
  poll()

  // Layer 3 — recovery: if window gets focus and we still have no port, retry immediately
  window.addEventListener('focus', () => {
    if (!(window as any).__ECHO_PORT__) {
      pollAttempts = 0
      poll()
    }
  })
} else {
  // Plain browser dev (no Electron): HTTP probe
  const tryPorts = async () => {
    for (const p of [8000, 8001, 8002]) {
      try {
        const r = await fetch(`http://127.0.0.1:${p}/health`, { signal: AbortSignal.timeout(500) })
        if (r.ok) { broadcastPort(p); return }
      } catch { /* try next */ }
    }
    setTimeout(tryPorts, 1000)
  }
  tryPorts()
}

ReactDOM.createRoot(document.getElementById('root') as HTMLElement).render(
  <React.StrictMode>
    <App />
  </React.StrictMode>
)

          ...
          return (
            <g key={kf.frame}
               ...
               onClick={e => {
                 ...
                 window.dispatchEvent(new CustomEvent('echo:seek', { detail: kf.frame }));
               }}
               onContextMenu={e => {
...
function ParamRowWidget({ param, clipId, currentFrame, onChange, onRefresh }: ParamRowProps) {
  ...
  const handleKfNav = useCallback((dir: 'prev' | 'next') => {
    ...
    if (dir === 'prev') {
      ...
      if (prev != null) window.dispatchEvent(new CustomEvent('echo:seek', { detail: prev }));
    } else {
      const next = frames.find(f => f > currentFrame);
      if (next != null) window.dispatchEvent(new CustomEvent('echo:seek', { detail: next }));
    }
...
function MasksPanel({ clipId }: { clipId: string }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('echo:masks-changed', handler);
    return () => window.removeEventListener('echo:masks-changed', handler);
  }, [clipId, load]);
...
export default function InspectorPanel() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('echo:frame', handler);
    window.addEventListener('echo:seek',  handler);
    return () => {
      if (timer) clearTimeout(timer);
      window.removeEventListener('echo:frame', handler);
      window.removeEventListener('echo:seek',  handler);
    };
  }, []);
  ...
  useEffect(() => {
    window.addEventListener('echo:masks-changed', refresh as EventListener);
    return () => window.removeEventListener('echo:masks-changed', refresh as EventListener);
  }, [refresh]);

...
const TrackRow = memo(function TrackRow({ track, trackIndex, scrollLeft = 0 }: Props) {
  ...
  const onDragOver = useCallback((e: React.DragEvent<HTMLDivElement>) => {
    if (e.dataTransfer.types.includes('application/echo-asset') ||
        e.dataTransfer.types.includes('application/echo-transition') ||
        e.dataTransfer.types.includes('application/echo-comp') ||
        e.dataTransfer.types.includes('application/echo-webcomp') ||
        e.dataTransfer.types.includes('application/echo-scene-hit') ||
        e.dataTransfer.types.includes('text/echo-scene-hit')) {
      e.preventDefault();
  ...
  const onDrop = useCallback(async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const transTypeId = e.dataTransfer.getData('application/echo-transition');
    if (transTypeId) {
    ...
    // Handle Comp Drop  
    const rawComp = e.dataTransfer.getData('application/echo-comp');
    if (rawComp) {
      let compMeta;
    ...
    // Handle WebComp Drop
    const rawWebComp = e.dataTransfer.getData('application/echo-webcomp');
    if (rawWebComp) {
    ...
    const rawSceneHit =
      e.dataTransfer.getData('application/echo-scene-hit') ||
      e.dataTransfer.getData('text/echo-scene-hit');  // Electron fallback
    if (rawSceneHit) {
    ...
    // Handle Asset Drop
    const rawAsset = e.dataTransfer.getData('application/echo-asset');
    if (!rawAsset) return;

...
export async function saveProjectTo(filepath: string): Promise<boolean> {
  // filepath may be the anchor .echo file  
  const folderPath = filepath.endsWith('project.echo')
    ? filepath.replace(/[\\/]project\.echo$/, '')
    : filepath
...
export async function loadProject(): Promise<LoadResult | null> {
  ...
  // Try folder picker first; fall back to .echo file picker
  ...
  if (!filepath) {
    filepath = await el?.showOpenDialog({
      filters: [{ name: 'Echo Project', extensions: ['echo'] }],
    })
  }
